...
$db=mysqli_connect("localhost","presi","1234","pp");
...
echo $id;
    ...
    $q2="select * from '$id' order by (pref) desc ";
        ...
        {	
            if ($r3['branch']!=NUll && $r3['branch']==$R['branch'])
            {
                if ($r3['gender']!=NUll && $r3['gender']==$R['gender'])
                {
                    ...
                    {
                        $q4="insert into allotment (id, company) values ('$id','$cid')";
                        ...
                        $q6="select * from allotment where id='&id'";
